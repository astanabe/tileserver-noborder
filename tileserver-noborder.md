...
for STYLE in maptiler-toner-en maptiler-basic-en; do
    echo "=== ${STYLE} ==="
    jq -r '
      [ .layers[] | .layout["text-field"] | select(. != null) ] as $tf
      | ($tf | map(select(type=="string" and test("name"))) | length) as $bare
      | ($tf | map(select(tostring | test("name:nonlatin"))) | length) as $nonlatin
      | ($tf | map(select(tostring | test("name:en"))) | length) as $en
      | "name:en 参照=\($en)  bare {name:*}=\($bare)  name:nonlatin 残存=\($nonlatin)  -> " +
        (if $bare==0 and $nonlatin==0 and $en>0 then "OK" else "NG (未書き換え)" end)
    ' $HOME/tileserver-gl/data/styles/${STYLE}/style.json
done
```
