...
> **プレースホルダ**: `etc/`、`data/`、`web/` 以下のファイルにはデフォルト値 (`tile.hogehoge.com`、`shimotsuki`、`/work/shimotsuki/planetiler` 等) が直書きされているが、これらは「テンプレート値」であって render プロセスで置換される。手動で `sed` する必要はない (旧版の運用)。
...
本リポジトリの `etc/`、`data/`、`web/` 以下のファイルにはデフォルト値 (`shimotsuki` / `tile.hogehoge.com` / `/work/shimotsuki/planetiler` 等) が直書きされている。別環境にデプロイする場合や設定を後から変更したい場合は、**ソースを直接編集せず**、`deploy.env` に変数値を書いて `scripts/render-configs.sh` で `staging/` に展開する。
...
| 変数 | 必須 | 用途 |
|---|---|---|
| `USER_NAME` | ✓ | systemd の `User=`/`Group=`、sudoers entry、各 `/home/$USER_NAME/...` パス |
...
```bash
. "$REPO/deploy.env"   # $BUILD_ROOT を読み込む
...
リポジトリ内の `scripts/buffer_clip.py` を使用する。入力はリポジトリ収録の GeoJSON。出力先は明示指定が必要 (環境ハードコードを避けるため、既定値は持たない)。

| 引数 | 必須 | 意味 |
|---|---|---|
| `--inputs` | ✓ | 入力 GeoJSON (複数指定可、和集合を取る) |
| `--buffer-m` | (既定 2000) | バッファ幅 (メートル) |
| `--out` | ✓ | 出力 `.poly` パス |
...
- `$BUILD_ROOT/build/islands_buffered.geojson` — 目視検証用のバッファ結果 (QGIS 等で確認)

**検証:** バッファが実際に島から 2 km 拡張されていることを確認 (`scripts/verify_buffer.py` は択捉島北端の南北 1 km / 3 km の 2 点を内外判定する。両方期待通りなら exit 0)。

```bash
"$REPO/scripts/verify_buffer.py" --geojson "$BUILD_ROOT/build/islands_buffered.geojson"
```
...
tileserver-gl はループバック専用バインドとし、外部公開は後段の nginx に任せる (TLS 終端と CORS / キャッシュを nginx 側で一元管理するため)。render 出力の systemd ユニットを配置する:
...
OSM 本体は日々更新されるため、週次でリビルドを行う。島嶼ポリゴン (`$REPO/geojson/`) は初回セットアップ時に手元に取得済み (§5)、以降は週次 rebuild で再利用するのみで OSM.jp は週次ループに登場しない (§1.4 参照)。ビルド処理は `$USER_NAME` 権限で実施し、MBTiles の置換と tileserver-gl の再起動、および nginx キャッシュ破棄のみ特権操作とする。
