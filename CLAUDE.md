...
> - **Exception: `tileserver-noborder.md` prose remains Japanese** — it is the user-facing spec (see "Editing conventions" below). Only its embedded code/comments are English.
...
> **重要 / IMPORTANT — Documentation policy:**
> - Repository files (`tileserver-noborder.md`, `README.md`, this file, script/config comments) describe the **current implementation only**. Do not add change history, migration notes, "previously / no longer / instead of the old way" framing, or statements whose only purpose is contrast with an earlier design.
> - **Change history and the reasoning behind a change go in the git commit message.** When a change needs explanation (what was removed and why, what behavior it replaces, what motivated it), write that in the commit body — not in the files being changed.

## Repository nature

This repo holds a deployable toolset (Python + Bash scripts plus systemd / nginx / sudoers / tileserver-gl configs) and one canonical document, `tileserver-noborder.md`, that walks through using them to stand up a self-hosted MapLibre vector + raster tile server on Ubuntu 24.04 / 26.04.
...
│                   lon/lat coordinates in scripts/rebuild.sh.
...
- **OS-version branches.** Where 24.04 and 26.04 differ (Node.js source, `canvas` rebuild, Mesa packages, nginx `http2` syntax) the doc carries both, tagged `[24.04]` / `[26.04]`, with a summary table right after the prerequisites table at the top. Keep the table and the tagged sections in sync.
- **Section numbering is load-bearing.** The doc cross-references sections heavily (e.g., "§1.2.4", "§8.3", "§9.2.1 (C)", "§11.1"). Renumbering or reordering sections requires updating every back-reference — in the doc, in `README.md`, in this file, and in script/config comments (`grep -rn '§' scripts etc`).
...
Two design decisions run through the spec and explain choices that otherwise look arbitrary. Read all of the places listed before changing anything substantive.

### 1. Self-contained build inputs

- **Style + sprite** come from the upstream `openmaptiles/maptiler-{basic,toner}-gl-style` GitHub repos at pinned tags (BSD 3-Clause + CC-BY 4.0). Fonts are instanced from `google/fonts` variable TTFs by `scripts/build_fonts.sh`.
- **Disputed-area regions** are **explicit lon/lat coordinates** in `scripts/rebuild.sh` (Northern Territories = an 8-point `buffer_clip.py --polygon`; Takeshima and Senkaku = `--bbox` rectangles). `buffer_clip.py` takes no GeoJSON input and applies no buffer.
- The yearly rebuild's only network access is the planet PBF download (`planet.passportcontrol.net`, `planet.openstreetmap.org` fallback) and Planetiler's `--download` of coastline / Natural Earth data. Tile serving itself makes no outbound connections; certbot renewal is the only recurring external contact.

Practical consequence: when changing the disputed-area regions, edit the coordinates in `scripts/rebuild.sh` and keep §5 / §1.4 in sync. Adding a style `source`, an `<img>` overlay, or any other external URL to the build or the served styles changes this design and must be deliberate.
...
- **§1.3 / §6.2** — disputed-island handling: within the region (defined by coordinates in `rebuild.sh`), geometry is KEPT but only the *text*-producing tags are stripped (`name`/`name:*`/`alt_name`/route `ref`/`addr:housenumber`) via `scripts/strip_island_labels.py`. The islands render their rivers/terrain/roads/buildings but carry NO labels. Pipeline (rebuild.sh steps 3/3.5/3.6): clip the body free of island features (`osmium extract -p` + `residual_label_ids.py` removeid), then extract the island region, strip text tags through OPL, and `osmium merge` it back.
- **§8.3 `patch_style.py`** — the style-rewrite that (a) adds `["!=", "maritime", 1]` to every `boundary` layer, (b) deletes the dedicated country-border layers and merges `admin_level=2,3` into the state-border layer (`admin_sub` / `boundary_state`) so country borders render identically to prefecture/state borders, (c) caps `place.class=country` labels at `maxzoom=5`, (d) rewrites every name-bearing `text-field` to `["coalesce", ["get","name:en"], ["get","name:latin"]]`, and (e) `mask_sea_boundaries()` moves the opaque `water` fill above the boundary layers then lifts `transportation` back above `water` — masking EVERY over-water boundary line (the `maritime!=1` filter only catches `maritime=1`; internal strait prefecture borders and the Hokkaido↔Northern Territories line are `maritime=0`) while keeping sea-crossing bridges/tunnels visible. This is the actual catch-all; (a) is only a first pass. Move-only, idempotent, no net layer change.
- **§9.2.1** — the verification recipes that prove (A) country-only border layers are gone, (B) `admin_level=2,3,4` are merged, (C) `maritime` filter is present on the merged layer, (D) country labels are zoom-capped, (E) layer order is `boundary < water < transportation` (the sea-mask), (F) text-fields prefer `name:en`.
- **§11** — troubleshooting maps user-visible symptoms back to which of (A)–(F) is broken.
...
planet.osm.pbf  (planet.passportcontrol.net mirror, OSM.org fallback)
   ...
   → residual removeid (§6.1)                   → clipped clean of island features
...
`--transliterate=false` is intentional and explained in §7: it disables ICU romanization so that names without `name:en` render as **empty** rather than as low-quality auto-transliterated Latin. Removing the flag would be a quality regression, not a cleanup.
...
- For doc edits to a numbered section: grep the doc for old-numbered back-references (e.g., `§8.3`, `§9.2.1`) and update them.
- For `scripts/*.py` edits: at minimum run `python -c "import ast; ast.parse(open('scripts/foo.py').read())"` and `scripts/foo.py --help`. `patch_style.py` can be checked end-to-end without a server: patch a fresh upstream `style.json` and confirm the output equals the deployed one (or that re-patching the deployed one is a fixed point). The full integration test is the deployment process itself (run on a target host).
